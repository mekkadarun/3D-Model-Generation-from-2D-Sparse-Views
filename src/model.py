...
# Generator: transform a latent vector into a 3D voxel grid representation.(64^3)
class _G(torch.nn.Module):
    def __init__(self, args):
        ...
        if self.cube_len == 32:
            padd = (1,1,1)
        # output: [batch_size, cube_len*8, 4, 4, 4]
        ...
        # output: [batch_size, cube_len*4, 8, 8, 8]
        ...
        # output: [batch_size, cube_len*2, 16, 16, 16]
        ...
        # output: [batch_size, cube_len, 32, 32, 32]
    ...
    def forward(self, x):
        ...
        #print(out.size())  # torch.Size([100, self.args.z_size, 1, 1, 1])
        out = self.layer1(out)
...
class _E(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.FC1 = torch.nn.Linear(400*input*input,self.args.z_size)
        self.FC2 = torch.nn.Linear(400*input*input, self.args.z_size)
...
class _E_MultiView(torch.nn.Module):
    ...
    def forward(self,images):
        means = var_or_cuda( torch.zeros(self.args.num_views,self.args.batch_size,self.args.z_size) ) # store the mean vectors for each view (shape: [num_views, batch_size, self.args.z_size])
        vars = var_or_cuda( torch.zeros(self.args.num_views,self.args.batch_size,self.args.z_size) ) # store the log variance vectors for each view
        zs = var_or_cuda( torch.zeros(self.args.num_views,self.args.batch_size,self.args.z_size) ) # store the sampled latent vectors for each view
        ...
        # latent vector: [batch_size, self.args.z_size] combine all view into one
